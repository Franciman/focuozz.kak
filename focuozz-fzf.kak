...
define-command -override focuozz-files %{
    fzf -preview -kak-cmd %{focuozz-filepath} -items-cmd 'find -L . -type f'
}

define-command -override focuozz-git %{
    fzf -preview -kak-cmd %{focuozz-filepath} -items-cmd 'git ls-files'
}

define-command -override focuozz-rg %{
    fzf -kak-cmd %{evaluate-commands} -items-cmd "rg --line-number --no-column --no-heading --color=never '' 2>/dev/null" -filter %{sed -E 's/([^:]+):([^:]+):.*/focuozz-filepath \1; execute-keys \2gvc/'}
}
